...
| `graph-kickoff` | Starting a fresh idea through the spec-forming half (nodes 1–4), stopping at gate-qc PASS with a docs proposal PR | `PRD/work/<slug>/GRAPH-RUN.md` ledger, package README `## Autonomous metadata` and `## Preparation gate`, status transitions, gate parks | `graph-preflight`, the boundary hook (`scripts/graph-boundary-hook.mjs`, always on), then `thejudge-kickoff`, `-refinement`, `-quality-check` — plus a docs-only PR into `main` the owner answers and merges |
| `graph-gate-review` | After a run parks at the `define` gate, to walk the recorded proposal one stable ID at a time | `GRAPH-RUN.md`'s `## Gate verdicts` and resolved `## Open gate`, the restored `STATUS.*` marker and board row, finalized verdicts inside `GATE-QUESTIONS.md`, and `DESIGN-BRIEF.md` (plus the README's intake pointer) reconciled to every `edit` or `reject` with a `### Brief reconciliation` list — never `PRD/sections/`, never `intake/` | nothing; it never dispatches and never advances a node |
| `graph-implement` | A single background loop that builds each approved-and-merged spec (nodes 5–9), one at a time, in one worktree on one branch cut from `origin/main` | One code PR per spec into `main`, carrying the code, the applied truth, the receipt, and the package deletion (`close` runs before `land`); `GRAPH-RUN.md` ledger, status transitions, gate parks | `graph-gate-review`, then `thejudge-map-out`, `-implement-all`, `-cleanup` — plus a no-write reviewer subagent at node 7 |
