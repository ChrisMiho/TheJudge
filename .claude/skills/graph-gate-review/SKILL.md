...
description: >-
  ...
  stable ID, applies them to the recorded PRD/sections/ diff, records the
  verdicts, resolves the gate, and hands back the exact command that resumes the
  run.
---
...
4. The current `PRD/sections/` files the diff touches
5. `PRD/instructions/graph-workflow-contract.md`

## Refuse unless the gate is an answered `define` diff

Read `## Open gate` first. This skill understands exactly one gate: a `define`
node's non-empty `PRD/sections/` diff, recorded as `GATE-QUESTIONS.md`.
...
3. Apply each verdict, in the order the IDs appear, inside that ID's recorded diff
   only:
   - `accept` — nothing; the run's text stands.
   - `edit` — apply the owner's `Reason:` as the correction to that ID's body.
   - `reject` — revert that ID out of `PRD/sections/` entirely; the number is
     burned and never reissued.
...
| `edit` | apply the owner's `Reason:` correction, inside that ID's body only | ID, `edit`, and the owner's reason quoted |
...
for any consumed ID. Renumbering would break every reference that already pointed
at it, which is the reason stable IDs are stable.
...
- `GRAPH-RUN.md` `## Open gate` — marked resolved, with the date and the verdict
  count. The recorded diff and the answered `GATE-QUESTIONS.md` stay; they are the
  evidence of what was reviewed.
- `PRD/sections/` — **only** to apply an `edit` or a `reject`, and only inside the
  recorded diff.
