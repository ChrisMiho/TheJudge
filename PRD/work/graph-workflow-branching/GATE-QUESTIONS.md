...
- Acceptance Criteria:
  ...
  - candidates are: local `thejudge-auto/*` branches whose tip is an ancestor of `origin/main` and whose package folder `PRD/work/<slug>/` (slug = the name after the prefix, with any `-work` / `-cleanup` suffix removed) no longer exists on `origin/main`; worktrees under `.worktrees/` whose branch is merged into `origin/main` and whose `git status --porcelain` is empty; `.worktrees/.graph-intake/<run-id>/` folders whose run id does not match the live `.worktrees/.graph-run.lock`
