...
- The corpus-scope decisions the intake calls "decided" (rules-only scope, local bundled embedding model, no vector database, hybrid lexical fallback) are product decisions and each gets its own gate slot with its evidence inlined; none is adopted silently.
