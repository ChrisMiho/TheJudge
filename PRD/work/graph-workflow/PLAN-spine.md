...
- ~~**Never modify any existing `thejudge-*` skill** in this plan.~~ **REVOKED 2026-08-14 by the user.** The original braindump forbade changing the existing skills. That constraint was load-bearing and wrong: it produced a driver whose control predicate matched nothing, because the four phase skills recognize only `thejudge-prepare is controlling`. Honoring it literally made unattended operation impossible. The user's ruling: keeping a parallel pipeline alive "only creates clutter," so the existing skills are to be updated to support the graph. Graph work **may** edit `.cursor/skills/thejudge-*/` and the instruction contracts (`preparation-contract.md`, `workflow-reference.md`). Prefer additive widening over rewrites; still run `npm run skills:ai-sync`, verify all three trees, and re-run affected fixtures per `AGENT-SKILLS.md`.
