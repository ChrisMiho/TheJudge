...
**Outcome:** A short "ding" plays on each successful auto-add, on by default, with a simple mute toggle on the scan screen. Muting suppresses sound without suppressing the visual confirmation. Generated via the Web Audio API (no committed binary asset) or an equivalently lightweight built-in.

**Non-goals:** No change to the visual confirmation popup, the stabilizer/lock logic, the add path, or any backend/API/prompt/`GameContext` boundary (DEC-050). No animation/audio library. Frontend-only.
