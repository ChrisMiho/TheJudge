...
2026-09-05 E1 — observed Slices A, B, and C all `done` (10/10, 6/6, 8/8
criteria `true` respectively; committed `323931e`, `c97a12f`, `4a9c0c2` on
`thejudge-auto/hybrid-rule-retrieval-work`) before starting this slice's code
and doc changes.
...
- [x] E1 — Slices A, B, and C are all `done` before this slice's code and doc
      changes land
- [x] E2 — the deployed Lambda's environment sets `EMBEDDING_PROVIDER=local`
      explicitly, recorded in the same places `ASK_AI_PROVIDER` is set
      (`scripts/aws-deploy.sh`, `scripts/aws-bootstrap.sh`)
- [x] E3 — `EMBEDDING_PROVIDER` unset still resolves to `mock` and never
      auto-switches on `NODE_ENV` or deploy target (existing coverage:
      `apps/backend/src/config/index.test.ts`)
- [x] E4 — a test asserts the deploy fails, rather than silently degrading,
      when the packaged model cache is absent
- [x] E5 — a local `npm run dev` with no warmed model cache and no network
...
- [x] Durable outcomes promoted; `PRD/work/hybrid-rule-retrieval/` ready to
      delete
