...
- Current node: `gate-qc` (attempt 6 — re-grade after the owner resolved the attempt-5 park by fixing the brief; build half, driven by `graph-implement`)
...
- **State:** RESOLVED 2026-09-09 by the owner ("Fix the two brief passages and resume the run"): both `DESIGN-BRIEF.md` passages rewritten to the accepted foil rule, `STATUS.refined` restored, run resumes at `gate-qc` attempt 6. The park record below is kept as history.
