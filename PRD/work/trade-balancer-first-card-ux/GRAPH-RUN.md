...
- Current node: `owner-action` (PARKED after `gate-qc` attempt 5 FAIL — the run's fourth FAIL; build half, driven by `graph-implement`)
- Next action: the owner resolves the gate under `## Open gate` (fix the two `DESIGN-BRIEF.md` passages on `thejudge-auto/trade-balancer-first-card-ux-work`, restore `STATUS.refined`), then `/graph-implement PRD/work/trade-balancer-first-card-ux/` re-enters at `gate-qc`
