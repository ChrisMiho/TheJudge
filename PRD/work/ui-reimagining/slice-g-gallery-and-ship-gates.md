...
One entry point for the owner's review — `mockups/index.html`, linking every
flow mockup and the Life Tracker before/after pair — and confirmation that
...
1. `mockups/index.html` links to `shared-chrome-menu.html`,
   `quick-question.html`, `in-depth-question.html`, and `trade-balancer.html`,
   plus the Life Tracker before/after pair (`mockups/before/life-tracker-*.png`
   next to `mockups/after/life-tracker-*.png`) — each entry labeled with its
   flow name and a note that it is viewable at phone (390x844) and desktop
   (1440x900) width.
...
5. The package `README.md` names `mockups/index.html` as the one entry point
   for the owner's review.

## Acceptance criteria

- [ ] G1 — `mockups/index.html` links all four flow mockup pages and the Life
      Tracker before/after pair, each labeled with its flow name and viewport
      note.
...
- [ ] G5 — `git diff <merge-base>...HEAD -- apps/frontend apps/backend` is
      empty.
- [ ] G6 — the package `README.md` names `mockups/index.html` as the entry
      point for the owner's review.
...
ls PRD/work/ui-reimagining/mockups/index.html
```

## Files touched

- `PRD/work/ui-reimagining/mockups/index.html` (new)
...
- [ ] Durable outcomes promoted; `PRD/work/ui-reimagining/` ready to delete
