...
| `PRD/sections/user-flows.md:252` | 4. After expanding the outer "General rules topics" disclosure, each topic row shows its t… | not this cap — System 2 curated topic excerpts in the local browse list, no per-request count |
...
| `PRD/sections/user-flows.md:518` | 3. User adds each card they want to discuss by typed autocomplete search (REQ-001/REQ-002)… | not this cap — 5-card attach limit (REQ-167) |
| `PRD/sections/user-flows.md:525` | - user tries to add beyond the cap → the add is blocked with a stated limit, mirroring exi… | not this cap — 5-card attach limit (REQ-167) |
...
| `PRD/sections/screen-layout.md:132` | \| Fit \| No page scroll for primary submit path — this bounds card image growth (REQ-129)… | not this cap — layout/size cap |
...
| `PRD/sections/system-map.md:291` | - Summary: Offline build that generates `cardhashes.bin` + manifest from Scryfall images u… | not this cap — offline build-script run budget (`--limit` / `--max-minutes`) |
| `PRD/sections/system-map.md:309` | - Detector robustness (shipped 2026-06-25, `scan-detector-foil-robustness`): the recall fi… | not this cap — scan detector candidate threshold |
...
| `PRD/sections/integrations-and-data.md:395` | - use per-card and whole-section caps so `MAX_PROMPT_CHAR_BUDGET` remains authoritative | not this cap — prompt character-budget caps (DEC-042), counted in characters not excerpts |
...
| `PRD/sections/decisions.md:140` | \| DEC-099 \| retired \| Rules Lookup is a reuse-first Ask AI entry for general rules ques… | not this cap — System 2 curated topic excerpts reused by the local browse fallback, no count |
...
| `PRD/sections/decisions.md:168` | \| DEC-127 \| retired \| The conversation thread becomes a full-bleed chat surface within … | not this cap — layout/size cap (conversation thread height) |
...
| `PRD/sections/decisions.md:207` | \| DEC-166 \| retired \| Graph boundaries move from the launch-flag permission profile int… | not this cap — graph per-node tool-call cap |
...
| `PRD/sections/scan/README.md:304` | replaced the capped selectable list with a single non-selectable "locking on: | not this cap — scan lock-on UI list bound |
| `PRD/sections/functional-requirements.md:138` | - Description: The core product must cap the stack at 10 cards. | not this cap — 10-card stack cap (DEC-008) |
...
| `PRD/sections/functional-requirements.md:356` | - excerpts are verbatim WotC CR prose for rule numbers listed in `apps/backend/data/gameRu… | not this cap — System 2 curated topic excerpts, which carry no per-request count |
...
| `PRD/sections/functional-requirements.md:1199` | - automatic responsive presentation (REQ-096 / DEC-117) adjusts surrounding spacing withou… | not this cap — layout/size cap |
| `PRD/sections/functional-requirements.md:1246` | - expanded `ScanReviewBubble` uses a 320px width with a viewport-safe cap and retains its … | not this cap — layout/size cap |
...
| `PRD/sections/functional-requirements.md:1836` | - Description: Quick Lookup must offer a small always-local list of core rules topics (lab… | not this cap — System 2 curated topic excerpts in the local browse list, no per-request count |
| `PRD/sections/functional-requirements.md:1841` | - expanding a row reveals that topic's rule numbers and excerpt; opening one topic auto-co… | not this cap — accordion display rule: one curated topic excerpt open at a time, not a retrieval count |
| `PRD/sections/functional-requirements.md:1842` | - the topic content is a committed frontend subset of the same curated `gameRulesByTopic` … | not this cap — System 2 curated topic excerpts, which carry no per-request count |
...
| `PRD/sections/functional-requirements.md:2211` | - for `mode: "lookup"`, combo retrieval runs only when combo intent is explicit and at lea… | not this cap — 5-card attach limit (REQ-167) |
...
| `PRD/sections/functional-requirements.md:2383` | - no per-player themes, Magic mana symbols/logos/card art, customization of the five fixed… | not this cap — the five fixed theme profiles, not a count of excerpts |
...
| `PRD/sections/functional-requirements.md:2544` | - Description: The shared conversation thread must fill the workspace's available vertical… | not this cap — layout/size cap (conversation thread height) |
| `PRD/sections/functional-requirements.md:2546` | - the message thread expands to use the conversation workspace's available height instead … | not this cap — layout/size cap (conversation thread height) |
...
| `PRD/sections/functional-requirements.md:3107` | - where container-relative sizing (DEC-160) would violate any criterion above on a given s… | not this cap — layout/size cap |
...
| `PRD/sections/functional-requirements.md:3298` | - Description: In the In-Depth Game Context commander-damage section, tighten the arrangem… | not this cap — commander-damage numeric input has no value cap |
| `PRD/sections/functional-requirements.md:3301` | - the field remains a free-typed numeric input (no dropdown, no min/max cap) since command… | not this cap — commander-damage numeric input has no value cap |
...
| `PRD/sections/functional-requirements.md:3379` | - exactly one sizing rule lives in the shared component — no per-screen fork, no size vari… | not this cap — layout/size cap |
...
| `PRD/sections/functional-requirements.md:3584` | - the existing loop cap is unchanged: `review` may return to `build` at most twice, and a … | not this cap — graph loop limit on node dispatch |
...
| `PRD/sections/functional-requirements.md:3600` | - the hook increments a counter keyed by run id, node, and attempt on each tool call, and … | not this cap — graph per-node tool-call cap |
...
| `PRD/sections/functional-requirements.md:3604` | - the cap adds no third loop limit: the contract's existing loop caps — at most three FAIL… | not this cap — graph loop limit on node dispatch |
| `PRD/sections/functional-requirements.md:3605` | - the cap is not a fifth terminal state; an overrun uses the existing `PARKED` state | not this cap — graph per-node tool-call cap |
| `PRD/sections/functional-requirements.md:3607` | - the cap is tool calls, not spend: the Agent SDK's `max_turns` and `max_budget_usd` are n… | not this cap — graph per-node tool-call cap |
...
| `PRD/sections/functional-requirements.md:3669` | - the heartbeat is unavailable when the cap is degraded — a missing or unparseable `.workt… | not this cap — graph per-node tool-call cap |
...
| `PRD/sections/functional-requirements.md:3859` | - The pre-submit view lets the player add, preview, and remove more than one card; an expl… | not this cap — 5-card attach limit (REQ-167) |
...
| `PRD/sections/functional-requirements.md:3879` | - Screen-layout's "Quick Question — pre-submit" row records a **single-card** image cap (R… | not this cap — layout/size cap |
| `PRD/sections/functional-requirements.md:3881` | - Gate review (2026-08-30) tightened the add cap from a suggested ~6 to a fixed 5, and dir… | not this cap — 5-card attach limit (REQ-167) |
...
| `PRD/sections/functional-requirements.md:4158` | - `npm run test:eval` stays green; any golden prompt change is an intentional, reviewed co… | not this cap — names an excerpt with no count |
...
| `PRD/sections/functional-requirements.md:4325` | - the ten labelled eval fixtures (`cascade-keyword`, `combat-deathtouch`, `counterspell-st… | not this cap — the ten labelled eval fixtures, a fixture count |
...
| `PRD/sections/functional-requirements.md:4405` | - the prompt is the one a player's lookup would get: `preparePromptInput` receives the com… | not this cap — cap-agnostic wording in the eval instrument; true at any cap value |
...
| `PRD/sections/functional-requirements.md:4442` | - the full transcripts — assembled prompt, the cards attached to the lookup, what System 3… | not this cap — cap-agnostic wording in the eval instrument; true at any cap value |
...
| `PRD/sections/system-map/prompt-assembly.md:50` | infrastructure remains visible while current caps avoid dropping full card metadata. | not this cap — prompt character-budget caps (DEC-042), counted in characters not excerpts |
...
| `PRD/sections/goals-and-non-goals.md:61` | - **Mitigation (planned):** context-driven System 2 topic selection (DEC-045) reduces base… | not this cap — prompt character-budget caps (DEC-042), counted in characters not excerpts |
...
| `PRD/sections/decisions/deployment.md:31` | - Context: The committed Commander Spellbook combo artifacts (DEC-162) grow with the upstr… | not this cap — Lambda zip-upload size ceiling, a deploy/infra limit |
...
| `PRD/sections/non-functional-requirements.md:103` | - default run uses curated success fixtures: `full-context`, `cascade-keyword`, `state-bas… | not this cap — a fixture name (`near-cap-stack`), not a cap value |
...
| `PRD/sections/in-depth/README.md:302` | `ADDITIONAL RELEVANT RULE EXCERPTS` → `OFFICIAL RULINGS` → combo context (when | not this cap — names the prompt's section order, no count |
| `PRD/sections/in-depth/README.md:324` | excerpts from committed artifacts, selected by DEC-045's always-on core plus | not this cap — System 2 curated topic excerpts, which carry no per-request count |
...
| `PRD/sections/in-depth/README.md:453` | per `screen-layout.md`'s five In-Depth rows. (DEC-145 via shared chrome, NFR-001) | not this cap — the five In-Depth rows in the `screen-layout.md` catalog, a row count |
...
| `PRD/sections/in-depth/README.md:553` | the full file lists, `PRD/sections/screen-layout.md`'s five `#### In-Depth —` rows | not this cap — the five In-Depth rows in the `screen-layout.md` catalog, a row count |
| `PRD/sections/quick-lookup/README.md:56` | state before submit, and can be removed individually. An add attempted past the cap is blo… | not this cap — 5-card attach limit (REQ-167) |
| `PRD/sections/quick-lookup/README.md:74` | excerpts the prompt uses — one source of truth, no hand-authored second copy. | not this cap — System 2 curated topic excerpts, which carry no per-request count |
| `PRD/sections/quick-lookup/README.md:78` | expanding one row reveals its rule numbers and excerpt and auto-collapses any | not this cap — System 2 curated topic excerpts in the local browse list, no per-request count |
| `PRD/sections/quick-lookup/README.md:79` | other open topic (accordion — at most one excerpt visible at a time). Reading a | not this cap — accordion display rule: one curated topic excerpt open at a time, not a retrieval count |
...
| `PRD/sections/quick-lookup/README.md:318` | This carries the composed string — the raw textarea is capped at the product | not this cap — question/notes character cap |
...
| `PRD/sections/quick-lookup/README.md:327` | ending with assistant, per-message cap — shared with the main flow, not a | not this cap — conversation-history retention cap |
...
| `PRD/sections/quick-lookup/README.md:338` | (REQ-167); an add attempted past the cap is blocked with a stated limit | not this cap — 5-card attach limit (REQ-167) |
...
| `PRD/sections/shared-chrome/README.md:37` | Shared chrome is the sixth Phase A spec on purpose. The first five each had to | not this cap — the first five Phase A specs, a spec count |
...
| `PRD/sections/shared-chrome/README.md:370` | column), and the cap still binds on ultra-wide displays. (DEC-145, REQ-124, | not this cap — layout/size cap |
...
| `PRD/sections/shared-chrome/README.md:385` | - View Context overlay: phone bottom sheet caps so a dismissible scrim of **≥25% of | not this cap — layout/size cap |
| `PRD/sections/shared-chrome/README.md:400` | no `max-h-32` pixel cap; shell-column surfaces render ~300px at 390×844 (REQ-141's | not this cap — layout/size cap |
...
| `PRD/sections/shared-chrome/README.md:431` | Menu mid-game). DEC-137 capped the interactive box to the painted affordance and made hit-… | not this cap — layout/size cap |
