...
Add a top-right navigation menu (distinct from and non-overlapping with `ThemeControl`) and an `appMode` view switch that mounts either the Stack Assistant flow or the Trade Balancer, preserving each mode's in-session state. (REQ-067, DEC-089, FLOW-010)
