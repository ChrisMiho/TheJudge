# Slice D — Live containment + orientation verification

## Status: planned

## Goal

Prove, live in a running app rather than by reading code, that the seat map
from slices B and C actually stays inside the card at the two hardest player
counts, in both layout modes, at phone width — and that the on-card glyphs
read right-side-up from every seat, including the 90°/270° side-column seats.
This is the browser-observable-risk slice per
`PRD/instructions/runtime-process-hygiene.md`: responsive geometry and
...
2. With Playwright MCP, set the viewport to iPhone-portrait (~430×900) and, for
   ...
   screenshot, not a pixel-diff tool — that on every card: no commander-damage
   map cell is cut off by the card's edge, and the player-name pill is neither
   crushed into nor spilling past the gutter between cards.
3. Pick one card seated in a 90°/270° side column at 7 or 8 players (the left
   or right column in grid mode) and confirm its map glyphs (the damage
   ...
   fix it by counter-rotating the glyphs only (not the map itself, not the
   card), matching the life-number treatment already in `PlayerLifeCard.tsx`,
   then re-verify the same seat.
...
- [ ] D1: at 7 players, grid layout, iPhone-portrait (~430px), every card's
      commander-damage map and player-name pill are fully inside the card — no
      cell clipped, no name pill crushed or spilled into the gutter.
- [ ] D2: same check at 8 players, grid layout.
- [ ] D3: same check at 7 players, list layout.
- [ ] D4: same check at 8 players, list layout.
- [ ] D5: at a 90°/270° side-column seat (7 or 8 players), the map's glyphs
...
# combinations to PRD/work/life-tracker-seat-map/.playwright-mcp/
