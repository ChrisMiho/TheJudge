...
| --- | --- | --- | --- |
| [A](slice-a-seat-map-geometry.md) | Add the compact-horizontal-block builder to `lib/lifeTracker/seatMap.ts`, alongside the existing `buildSeatMapCells` (kept, unchanged, for the panel) | — | planned |
| [B](slice-b-card-preview-seat-map.md) | `PlayerLifeCard`: on-card preview switches to the compact-horizontal block (≤2 rows, grows wider), same shape in grid and list layout | A | planned |
