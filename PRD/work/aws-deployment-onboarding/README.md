...
|-------|------|--------|------------|-----|
| A | Adopt `aws_web_app` onto a feature branch off `main`; verify local build/tests green | planned | — | [slice-a](./slice-a-adopt-branch.md) |
| B | Gap 3 — `quality:check` deploy gate; AWS account ID → GitHub repo variable | planned | A | [slice-b](./slice-b-ci-gate-and-account-var.md) |
| C | Gap 1 — SSM SecureString + Lambda cold-start read; flip to `openai`; IAM grants | planned | A | [slice-c](./slice-c-openai-secret-ssm.md) |
| D | Gap 2 — Lambda reserved concurrency + AWS Budgets alarm | planned | A | [slice-d](./slice-d-cost-scale-guardrails.md) |
