...
+- Notes:
+  - measured 2026-09-06: all six cases carry a `workedSolution` of 187–592 characters and exactly one expected rule id; 31 eval fixtures exist, 10 carry an `expected` block, and 0 carry an answer; the committed rule index `apps/backend/data/gameRulesRuleIndex.json` holds 277 `Example:` lines across 215 of its 2,873 rule entries (re-measured 2026-09-07 in this checkout, where the raw CR download is absent as designed) and the committed rulings index holds 76,605 rulings over 19,542 cards — the two official pools the tiers draw from, both already committed and both already served in production
