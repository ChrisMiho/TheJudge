...
define-gate rule the run parks here for owner review before any code is written
against this product truth.

**New stable IDs minted: none.** This is Phase A #6; DEC-168 (the spec template)
already exists, so refinement minted zero new DEC/REQ/FLOW/NFR/Q. The spec is a
derived current-state view citing existing IDs only. `graph-gate-review` is
ID-centric, so its walk maps onto the spec's behavior sections (suite shell +
mock banner, routing + load fallback, Menu rail + tray, Theme, answered
workspace, history drawer, View Context overlay, card-detail popup + close
control, shared layout language, measured bounds, rejected alternatives) rather
than a list of new IDs. A new file is a non-empty diff — this is not an empty
diff that advances straight to `gate-qc`.

**Directory name to confirm at the gate:** `shared-chrome` is proposed, taken
from the existing `### Shared chrome` heading already in
`PRD/sections/screen-layout.md` (the in-repo naming signal, not a blind guess).
The request asked for this confirmation explicitly.

**SECOND OWNER ACTION — the branch is contaminated by a concurrent session.**
Node 1 reported the checkout clean. Mid-run, a **concurrent session sharing this
same checkout** made commit `0445150` (`feat: Add design briefs and refine status
for shared chrome and lambda-s3-deploy`) directly onto this run's branch
`thejudge-auto/shared-chrome-spec`. That single commit bundled **two packages'
work together**:

- This run's own work (legitimate on this branch): `PRD/sections/shared-chrome/README.md`,
  and the `PRD/work/shared-chrome-spec/` package.
- The separate `lambda-s3-deploy` package's work and **its product truth** (does
  NOT belong on this branch): DEC-169 (`PRD/sections/decisions.md` index +
  `PRD/sections/decisions/deployment.md` body), REQ-165
  (`PRD/sections/functional-requirements.md`), NFR-017
  (`PRD/sections/non-functional-requirements.md`), and the
  `PRD/work/lambda-s3-deploy/` package.

The run lock stops another *graph run*, not an interactive session or a manual
`git commit` in the same working directory, so nothing prevented this. The run
did **not** make commit `0445150` and did not stage any `lambda-s3-deploy` path;
its own follow-up commit stages only the shared-chrome package + spec + board row.
It did not attempt to untangle `0445150` — separating two packages from one commit
is history rewrite (`reset`/`rebase`/force-push), which a graph run may not do and
which is the owner's call.

**Why this must be resolved before the run continues past `gate-qc`.** When the
run later reaches `build`, it opens a PR whose base is
`origin/thejudge-auto/shared-chrome-spec`. On the branch as it stands, that PR
would carry the `lambda-s3-deploy` product truth (DEC-169 / REQ-165 / NFR-017)
into the shared-chrome deliverable. The owner should split `0445150` — land the
...
```diff
diff --git a/PRD/sections/shared-chrome/README.md b/PRD/sections/shared-chrome/README.md
new file mode 100644
index 0000000..de2f4ff
--- /dev/null
...
_Recorded as a 4-space-indented block so the prompt's own `##` headings do not split the ledger's sections. Every word is verbatim; only a uniform indent is added._

    graph-run is controlling.

    Working directory: /Users/chrismiho/Coding/Projects/TheJudge

    You are node 3 (`define`) of an autonomous graph run. Invoke the `thejudge-refinement` skill and follow it exactly. Do not run interactively or pause for user approval — graph-run is controlling. Copy the `Working directory:` line above, unchanged, into any prompt you write.

    Package: `PRD/work/shared-chrome-spec/`. Read its `README.md` and `IDEA.md` first — they carry the full backing-sources list, the reference-implementation pointers, and the non-goals.

    ## What this run is

    Phase A #6 of the docs-refactor gameplan: author the current-state feature spec for the **shared-chrome** bucket, landing at `PRD/sections/shared-chrome/README.md` on the **DEC-168** template.

    DEC-168 already exists (it was minted by Phase A #1, life-tracker). So this run mints **no new stable IDs** — the spec is a derived, current-state VIEW over existing DEC/REQ/FLOW/NFR truth, consolidated into one player-terms file. This matches the established pattern of the five shipped Phase A specs (`PRD/sections/life-tracker/`, `user-feedback/`, `trade-balancer/`, `scan/`, `quick-lookup/` — all DEC-168). Read those five as your worked templates for the section shape (`Status:` / `Backed by:` header, **What it is**, **How it works**, **Measured bounds**, **Rejected alternatives and deferred scope**, **Where it lives**).

    ## Framing constraints (mechanics/framing, from the gameplan — not product decisions to re-litigate)

    - **Draft and non-authoritative.** The spec carries an explicit draft marker. `decisions.md` and the existing sources stay precedence #1 through Phase A/B. A spec that comes out wrong is corrected against sources that still exist.
    - **No new stable IDs.** Do not mint DEC/REQ/FLOW/NFR. Cite the existing ones in `Backed by:`.
    - **Author only the new file** `PRD/sections/shared-chrome/README.md` (a derived view). Do NOT edit `screen-layout.md`, `decisions/*`, `functional-requirements.md`, `user-flows.md`, `system-map.md`, or any other existing source body — restating/consolidating the shared-chrome rows and the shared layout language into the new spec is Phase A's job; physically retiring the source rows is Phase C's, not this run's. This keeps the `PRD/sections/` diff to one new file, exactly like the five prior specs.
    - **Measured bounds travel with their row** (gameplan binding constraint 8): every measurement on a shared-chrome surface that still exists in code must appear in the new spec's **Measured bounds** section. A row arriving stripped of its measurement is the failure this refactor exists to prevent.
    - **Directory name** `shared-chrome` is proposed, to be confirmed or corrected by the owner at the define gate. The package README notes `PRD/sections/screen-layout.md` already has a `### Shared chrome` heading grouping exactly these rows — that is the in-repo naming signal, not a blind guess.

    ## Scope of the bucket

    The shared layout language and chrome every destination mounts into, that the five feature specs kept reaching for without owning: suite shell, Feature-portal Menu rail + tray, mock-mode banner, destination/route load fallback, card-detail popup (suite-wide), conversation history drawer, View Context / adaptive context overlay, and the `## Shared layout language` (viewport bands, hybrid % model, fit rule, anti-overcalibration) — plus the `screen-layout.md` rows that belong to shared chrome rather than a single feature (gameplan binding constraint 7). Confirm the exact REQ set during refinement; several REQs in the backing list are per-feature and must stay out.

    ## Reading rules

    - You MAY read any `PRD/sections/**` source file — that is the product corpus you are consolidating.
    - Intake (`PRD/work/shared-chrome-spec/intake/refactor-gameplan.md`) is evidence, never authority. **Never open, read, or fetch any document the intake cites** (e.g. `workflow.md`, `workflow-decomposition.md`, `answers.md`) — record only their paths as citations.

    ## Product questions

    Apply the assumption ladder in `preparation-contract.md` **per question**, fresh at the moment it arises. A stated framing preference is an input to that ladder, not a bypass. If a genuine decision blocker under the three-condition test arises, park — do not guess product behavior on the owner's behalf.

    ## Output

    Produce `DESIGN-BRIEF.md` in the package and author `PRD/sections/shared-chrome/README.md`. Set the package STATUS per the refinement skill (`refining` while in flux, `refined` on completion).

    Report back concisely:
    - The spec file path written and whether STATUS reached `refined`.
    - Confirmation: zero new stable IDs minted; zero edits to existing source bodies.
    - The `Backed by:` ID set the spec cites.
    - Any assumption-ladder decisions made, and any genuine blocker that parked.
    - Do NOT write GRAPH-RUN.md — that is the driver's job.
