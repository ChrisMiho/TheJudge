...
| DEC-089 | `decisions/trade-balancer.md` | Top-level in-app navigation menu in the top-right header (distinct from and non-overlapping with `ThemeControl`) switches between Stack Assistant and Trade Balancer; a frontend-only view switch that preserves each mode's in-session state, no persistence across reload, no backend/contract change. |
