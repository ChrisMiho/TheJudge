...
| DEC-084 | `decisions/ui-presentation.md` | In mock provider mode the frontend shows a persistent, non-dismissible mock-mode banner on every screen; the signal is build-time configuration-driven from the single `ASK_AI_PROVIDER` source of truth (surfaced via `vite.config.ts` → `import.meta.env.VITE_ASK_AI_PROVIDER` → `env.ts`), never inferred at runtime and never adding a backend endpoint or contract change. |
| DEC-085 | `decisions/doc-process.md` | `npm run quality:check` runs the Vitest suite once (coverage-mode as the single regression + coverage gate, redundant standalone `test` step removed), and test files are reorganized for clarity (split `App.test.tsx`, extract shared EnrichmentStep fixtures, group scan suites) as an assertion-preserving refactor; thresholds, eval gate, and product behavior unchanged, cross-workspace parallelism deferred. |
| DEC-086 | `decisions/trade-balancer.md` | Card Trade Balancer: a standalone frontend-only ephemeral feature comparing two sides' total USD value (per-entry printing + foil toggle + quantity, scan or manual-search input, missing price → $0 with a caution indicator); narrows the pricing and printing-disambiguation non-goals; no backend/contract change. |
| DEC-087 | `decisions/trade-balancer.md` | The trade balancer is powered by a new committed, lazy-loaded, printing-level USD price artifact built offline from the Scryfall bulk source (per printing: usd/usd_foil + set/collector/image, indexable by oracle and printing id); a static snapshot with no runtime sync, refreshed only via the human-approved data pipeline. |
| DEC-088 | `decisions/trade-balancer.md` | Top-level in-app navigation menu in the top-right header (distinct from and non-overlapping with `ThemeControl`) switches between Stack Assistant and Trade Balancer; a frontend-only view switch that preserves each mode's in-session state, no persistence across reload, no backend/contract change. |
