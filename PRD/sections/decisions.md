...
| DEC-072 | `decisions/scanning.md` | Real-world detector robustness raises `detector.ts` recall (loosened/adaptive gates, foil-tolerant edge sourcing, low-contrast-border fallback) with the stabilizer lock gate as the precision guard, validated against a committed fixture corpus + a debug-gated Capture-button frame export; the Region A recipe and `CARDHSH1` bin stay frozen. Refines DEC-052/DEC-055; complements DEC-062/DEC-069. |
