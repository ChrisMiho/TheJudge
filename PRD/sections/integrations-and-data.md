...
- the planned committed backend artifacts are gzipped: `apps/backend/data/commanderSpellbookCombos.json.gz` (trimmed variant detail + source manifest) and `apps/backend/data/commanderSpellbookComboIndex.json.gz` (inverse oracle membership, template expansions, and unresolved-template metadata). Compression keeps the full corpus near 6.5 MB instead of ~112 MB, so no variant is dropped for size; the loader decompresses on first read
