...
- the planned committed backend artifacts are gzipped: `apps/backend/data/commanderSpellbookCombos.json.gz` (trimmed variant detail + source manifest) and `apps/backend/data/commanderSpellbookComboIndex.json.gz` (inverse oracle membership, template expansions, and unresolved-template metadata). Compression keeps the full corpus at 11.3 MB instead of 164 MB, so no variant is dropped for size; the loader decompresses on first read, which measured ~42 ms on top of parsing. Resident memory, not load time, is the constraint worth sizing (DEC-162)
