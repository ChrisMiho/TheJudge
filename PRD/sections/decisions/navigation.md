...
- Notes:
  ...
  - superseded in part by DEC-157: URL-based routing **is** introduced, and the URL — not `sessionStorage` — is the source of truth for the active destination. This decision's session-scoped `sessionStorage` semantics (guarded read, validation against registered ids, fallback to the first registered destination) survive as the bare-`/` fallback; its "no URL-based routing is introduced" clause does not.
...
### DEC-157
- Decision: The four registered feature-portal destinations become **addressable top-level URLs** using `react-router`, and the **URL becomes the source of truth** for which destination is active. This supersedes DEC-111/REQ-090's "no URL-based routing / react-router" constraint. `sessionStorage` is demoted from source of truth to the fallback consulted **only when the URL carries no destination** (a bare `/`), which preserves DEC-111's refresh-restore behavior for users who did not arrive by deep link. Routes are **flat and top-level only** — one path segment per destination, no nested or parameterized routes and no search-param state. The destination registry gains a `path` field so the id↔path mapping has exactly one home; registry order still supplies the menu order and the no-stored-preference default (DEC-135). `DestinationOutlet`'s keep-alive mounting is **preserved unchanged**: the router decides which destination id is active, never what stays mounted. Each destination additionally gains a `React.lazy` code boundary with its **own** `Suspense` boundary — one per destination, inside the outlet's per-destination wrapper, never a single boundary around the outlet, which would suspend and blank already-loaded siblings — plus explicit `manualChunks` groups for the scan code shared across destinations and for framework code.
- Status: confirmed
- Context: The frontend grew to four registered destinations switched by state inside a 117-line `App.tsx`, with no router and no code splitting — no `React.lazy`, dynamic `import()`, or `manualChunks` anywhere in `apps/frontend`. No function area was addressable: no deep links, no shareable URLs, no browser back/forward, and no way to land a user directly on a feature; and every visitor downloaded every feature's code regardless of what they opened. Both constraints tighten as TheJudge grows from one validated assistant loop into the planned multi-feature suite (DEC-094). DEC-111 had deliberately chosen `sessionStorage` over routing, but its own Notes scope that as a "non-goal of this decision itself" rather than a standing product prohibition. `react-router` was chosen over a minimal hash/history wrapper and over `@tanstack/react-router`: the wrapper would accrete exactly the nested-route, param, and search-param handling a library already solves, and TanStack's route generation and loader model buy nothing for four flat routes while being the larger reversal. Critically, `react-router`'s `<Routes>` **unmounts** non-matching routes, which would have silently broken DEC-095/REQ-067's in-session state-preservation guarantee — so the library supplies location reading and history only, not mounting policy.
- Impact:
  - `apps/frontend` gains `react-router` as its fifth runtime dependency; `<BrowserRouter>` wraps the app
  - registry entries carry `path`: `/quick-lookup`, `/in-depth`, `/life-tracker`, `/trade-balancer`. Paths are **declared literally per registry entry**, never derived from the id or the label — neither produces all four cleanly (`quick-lookup`'s id reads well as a path while `mtg-assistant` and `player-life-tracker` do not, and DEC-135's labels are prose, not slugs). Adding a destination means choosing a readable, stable path and declaring it alongside `id` and `label`.
  - `useActiveDestination` derives `activeDestinationId` from the URL and navigates on set, keeping its existing two-field public shape; `App.tsx` call sites are unchanged
  - a bare `/` resolves via the existing guarded `sessionStorage` load/validate/fallback helper, then the first registered destination; an unknown path redirects to `/`
  - selecting a destination pushes a history entry, so browser back/forward moves between destinations
  - `DestinationOutlet` keeps mounting every visited destination and hiding inactive ones with `hidden` — unchanged; a destination suspends only on its first visit, so the `Suspense` fallback appears once per destination rather than on every switch
  - under `<BrowserRouter>`, jsdom's `window.history`/`location` is file-global and leaks between test cases; the shared test setup must reset history alongside its existing storage cleanup, or destination-switching tests bleed URL state into their successors
  - `vite.config.ts` gains `build.rollupOptions.output.manualChunks` in **function form** — `(id) => ...` — not object form: object form maps a chunk name to explicit module ids and does not accept path patterns, so a glob written there fails the build. Two groups: a `scan` group covering the scan surface shared by all three scanning destinations, and a `vendor` group covering framework code. The scan surface is larger than `src/lib/scan/**`: `src/hooks/useScanCapture.ts` is imported by `portal/quick-lookup/QuickLookupApp.tsx`, `ZoneCollectionStep.tsx` (In-Depth), and `trade/useTradeScan.ts`, and `src/components/ScanCameraSurface.tsx` is imported by both `QuickLookupApp.tsx` and `trade/TradeSide.tsx`. The group is defined by measured import-graph membership, not by directory alone. The `vendor` group must include `react/jsx-runtime` (emitted by the automatic JSX transform, a distinct module id from `react`) and `react-router` alongside `react`/`react-dom`.
  - REQ-090's "a brand-new tab still opens on the first registered destination" now holds **only for a bare `/`**: a deep link to `/trade-balancer` in a brand-new tab lands on Trade Balancer, which is the intended point of the feature
  - the `player-life-tracker → mtg-assistant` roster-seed handoff stays in the selection handler, so a direct deep link to `/in-depth` correctly does not seed
  - **Send feedback** stays an action entry with no route (DEC-104); no scan, conversation-history, or portal-shell surface becomes a route
  - no change to `POST /api/ask-ai`, Zod schemas, `GameContext`, prompt assembly, provider selection, backend routes, card metadata, scan behavior, the data pipeline, or any portal chrome (DEC-122/DEC-133/DEC-150 unchanged)
  - no infrastructure change is required: CloudFront already maps 403/404 to `/index.html` with a `200` response code (`scripts/aws-bootstrap.sh`), so deep links resolve in production
- Related requirements:
  - REQ-140
  - NFR-014
  - REQ-090
  ...
  - NFR-012
- Notes:
  - supersedes only DEC-111/REQ-090's no-URL-routing constraint and their "sessionStorage is the source of truth" clause; DEC-111's session-scoped semantics survive as the bare-`/` fallback, and DEC-095's registry, in-session state preservation, and frontend-only switching are unchanged
  - in-flow state stays ephemeral: staged `contextFlow`, conversation, and follow-up state are **not** serialized into the URL — deep-linkable in-flow state would be new product behavior with its own privacy surface (game context in a shareable URL) and is a deliberate non-goal
  - non-goals: nested/parameterized routes, search-param state, server rendering or a framework migration, a state-management rewrite, any visual redesign, and regrouping the 19 `App.*.test.tsx` files along route boundaries (a separate NFR-012 hygiene item)
  - route-level splitting is a **code**-splitting change only; the existing data-artifact lazy loads (`cardhashes.bin` per NFR-010, `cardPrintingPrices.json` per NFR-013) are untouched
