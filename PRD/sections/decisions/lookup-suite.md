...
- Impact:
  ...
  - the composed `question` (pill phrase plus optional textarea text, space-joined, or the textarea alone when no pill is locked) is what reaches the backend; the shared REQ-011 300-character cap and the visible counter measure the **raw editable textarea content**, not the composed string (amended — see Notes)
...
- Notes:
  ...
  - **cap/counter measurement amended during the `ui-review` pass (2026-08-06)**: this decision's Impact originally read "measured against the shared REQ-011 300-character cap as one string, with the visible counter reflecting the composed length". Live measurement found that rule produces an empty field reporting `22/300`, a counter that rises on backspace-to-empty, and an unreachable `323/300` submit state. The cap and counter now measure raw editable text (REQ-091 as amended, REQ-134); the composed string may exceed 300 by the pill phrase, which is accepted against DEC-042's 1,000,000-char prompt budget. **Composition itself is entirely unchanged** — the pill phrase, space-join, single-pill swap rule, and silent `Tell me about {Card Name}.` fallback all still build the submitted `question` exactly as specified above.
