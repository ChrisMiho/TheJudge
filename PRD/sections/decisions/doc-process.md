...
- Impact:
  ...
  - jsdom becomes per-file opt-in via `environmentMatchGlobs`, extended only to globs a green run proves; blanket directory- or extension-based rules are prohibited because `src/lib/lifeTracker/useLifeTracker.test.ts`, `src/lib/feedback/FeedbackContextProvider.test.tsx`, and `src/lib/portal/seedContext.test.tsx` measurably still need a DOM
