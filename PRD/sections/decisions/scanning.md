...
### DEC-090
- Decision: The camera scan view gets a responsive, non-overlapping overlay layout for phones. Three presentation-only changes: (1) the debug toggle's corrupted `-tranzinc-x-1/2` utility is fixed to a valid centering transform, and the debug toggle and the "Powered by Cardomancer" watermark are guaranteed non-overlapping bounds and hit areas at all supported phone widths — extending DEC-065's non-overlap guarantee to include the watermark; (2) the `Exit scan` control is lifted out of the camera-feed overlay into a slim control row above the frame, so the top-right region DEC-065 reserves for the scanned-cards review bubble is no longer intruded upon and the crowded top edge is relieved; (3) the camera frame becomes responsive and grows to consume available viewport height on tall phones (replacing the fixed `aspect-[3/4]`), bounded by a max/min so the exit row and capture button stay visible without page scroll, with the alignment guide, lock outline (DEC-083), and debug overlay (DEC-060) scaling to the rendered frame. Refines the scan-screen layout in DEC-057/DEC-060/DEC-061/DEC-065/DEC-083 (placement only); supersedes none; changes no scanner behavior and is explicitly not a scan-robustness lever.
- Status: confirmed
- Context: Before this change, the small-viewport scan overlay had three concrete layout faults. The debug toggle used the corrupted `-tranzinc-x-1/2` class, so its left edge pinned at center and overlapped the bottom-right watermark. The status pill and mute toggle occupied the left edge while the parent's absolutely positioned `Exit scan` control intruded on both the status width and DEC-065's top-right review-bubble region. The fixed `aspect-[3/4]` video also stopped well short of the bottom of tall-phone viewports. Because the feed already used `object-cover`, growing the frame could show more vertical field without distortion. An inline "scanned this session" thumbnail strip was considered for the gap and set aside to preserve DEC-058's compact-bubble model.
- Impact:
  - the debug toggle is correctly centered and the debug toggle + Cardomancer watermark have non-overlapping bounds and pointer hit areas at ≤360px / 390px / 414px; this extends DEC-065's non-overlap guarantee (previously debug-toggle vs review-bubble) to also cover the watermark
  - `Exit scan` moves from a camera-feed overlay control into a slim control row above the frame; the top-right region stays reserved for the scanned-cards review bubble and its expanded remove affordances (DEC-058/DEC-065), and the status indicator + mute toggle no longer collide with it
  - the mute toggle is placed in the alignment guide's top-left corner (off the outline edge it previously crossed) and the Cardomancer watermark is moved up to sit centered inside the guide's lower edge; both render at ~90% opacity (10% transparent) so the card shows through slightly while they stay legible. This placement is overlay-only DOM and has no effect on detection/matching (the detector reads the raw camera frame, not the composited overlay)
  - the camera frame is responsive and fills available viewport height on tall phones, bounded so the exit row and capture button remain visible without page scroll; the `745/1040` alignment guide, the DEC-083 locking outline, and the DEC-060 debug overlay all scale to the rendered frame, and `object-cover` is preserved so the feed is cropped, not distorted
  - graceful degrade on short viewports and desktop: with no excess vertical space the frame falls back toward its prior sizing; nothing clips or introduces unexpected scroll
  - **no scanner-behavior change:** no change to detection/warp, the shared resize+DCT+hash recipe (`recipe.ts`), `cardhashes.bin`, the matching/orientation/distance logic (`identify.ts`), the stabilizer/lock gate (`lockDistance`/`marginMin`, DEC-059), auto-add (DEC-056), audio (DEC-061), the scanned-cards review/remove behavior (DEC-058), or the DEC-051/REQ-034 parity gates
  - **not a scan-robustness lever:** distinct from DEC-062 (query conditioning), DEC-069 (corpus), DEC-072/DEC-073 (detection recall), DEC-074 (capture resolution); this touches only overlay positioning and frame sizing
  - any layout motion stays CSS-only and `prefers-reduced-motion`-aware (NFR-006); no animation library and no new scan-screen control
