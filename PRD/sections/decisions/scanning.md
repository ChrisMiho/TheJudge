...
- Impact:
  ...
  - evidence + validation: a committed detector fixture corpus is the outcome bar — committed fixture seeds and/or committed generated synthetic degradations (glare/specular, low-contrast border, perspective skew, foil-like highlights) with recorded provenance as the controllable backbone, plus downloaded real-world card-on-table photos and an optional on-device raw-frame export; before/after detect-then-lock rate across the corpus and on-device is recorded as acceptance evidence
