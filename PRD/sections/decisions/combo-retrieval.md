...
- Impact:
  ...
  - the corpus covers every reviewed `OK` variant the export publishes — 106,182 in the committed 2026-08-22 snapshot, and the export regenerates daily so the exact count drifts; the export contains no `EXAMPLE` variants, so DEC-116's `EXAMPLE` rejection is retained defensively but is not exercised by this source
  - committed artifacts measure **76.9 MB detail + 4.8 MB index**, measured over the real 106,182-variant corpus. This is far above the 9.6 MB + 1.7 MB estimated from a 6,000-variant sample, and the cause is the storage format rather than a defect: gzipping each variant individually — required so a lookup never decompresses more than the one record it asked for — forfeits the cross-record compression a single shared gzip stream gets almost free across ~106k records with repeated JSON keys. Owner decision 2026-08-22: commit as measured. The memory-safety goal (a bounded per-request detail fetch instead of ~868 MB resident) is what the format buys; the larger repository footprint is an accepted cost, not deferred work
...
- Notes:
  ...
  - if repository size ever becomes an actual problem, the lever is batching several variants per gzip member instead of one each, trading some compression back for a coarser lazy-fetch granularity. Do not revisit it before then
  - the real 2026-08-22 refresh surfaced two failures no smaller-scale test could reach: the bulk document decompresses to ~634 MB, past V8's ~536 MB maximum string length, so `JSON.parse` over the whole document throws outright — both the refresh and the build now use a streaming JSON-array parser (`scripts/lib/stream-json-array.mjs`, the same technique already used for Scryfall's bulk file); and a single Scryfall template-expansion query that 404s no longer aborts the refresh, being treated exactly like a template with no query at all
