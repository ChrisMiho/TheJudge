...
- Main Flow:
  ...
  2. The pre-submit view shows, top to bottom: an optional card-attach control, the Question field, then the "General rules topics" section — a short list of core rules topics (the stack & priority, targeting, combat, layers) the user can read locally with no AI call. The topics section is always visible, regardless of whether a card is attached or the Question field already has text.
  ...
  6. Frontend sends `{ mode: "lookup", question, card? }` to `POST /api/ask-ai`; `question` is the client-composed string (the locked pill phrase plus any supplementary textarea text, or the textarea alone when no pill is locked); `card` is present only if one was attached; no `gameContext` is sent.
  ...
  10. User may start over, which clears the thread, any locked topic pill, and returns to the pre-ask state — with the looked-up card preserved if one was attached; the "General rules topics" section remains visible either way.
- Edge Cases:
  - if no pill is locked and the question is blank after trimming, submit is blocked; the "General rules topics" section remains visible regardless (it is not a fallback state, per REQ-079's always-visible behavior)
...
- Notes:
  ...
  - the "General rules topics" section's placement, always-visible gate removal, row-level accordion disclosure, and the "Use this topic" locked-pill mechanism were confirmed during quick-question-ui-refinement (DEC-112 / REQ-091)
