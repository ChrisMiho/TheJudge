...
- Acceptance Criteria:
  ...
  - for each ready spec the loop dispatches `graph-gate-review` to finalize the owner's verdicts and reconcile `DESIGN-BRIEF.md` (and the README's pointer to a verbatim `intake/` file) to every `edit` or `reject` — the brief is what the re-grade grades and `plan` slices, so it cannot be left on the pre-verdict rule — then re-enters at `gate-qc` and runs `plan → build → review → close`, every node in `.worktrees/implement-<slug>` on `thejudge-auto/<slug>-work` cut from `origin/main` (REQ-193)
...
- Acceptance Criteria:
  ...
  - the same holds for the second native module the local embedder needs: `@huggingface/transformers` imports `sharp` at module load, and `sharp` resolves its build from the optional packages `@img/sharp-linux-arm64` and `@img/sharp-libvips-linux-arm64`, which npm installs only for the platform *running the install* unless told otherwise — so `scripts/package-lambda.sh` runs `npm ci --os=linux --cpu=arm64 --libc=glibc` (the function's platform, not the deploy runner's linux/x64 or a laptop's darwin/arm64), and `scripts/lib/assert-sharp-platform-binding.mjs` refuses (exit 1) when the resulting `node_modules` lacks that build; `scripts/lib/assert-sharp-platform-binding.test.mjs` proves the refusal and the script's install flags
...
- Notes:
  ...
  - measured 2026-09-07 (answer-quality run 3, cards attached): every tier-2 prompt carried its cited ruling verbatim (Panharmonicon 9 rulings attached, Restoration Angel 3, Sensei's Divining Top 2) and every model answered all three tier-2 cases fully correctly at both caps except `gpt-5-nano` on the Sensei's Top case (Correctness 1 at cap 5, 0 at cap 10); in the two earlier runs, which asked the bare question, the same three cases were also mostly answered correctly — from memory, which is not what this tier tests
...
- Acceptance Criteria:
  ...
  - credentials load the way `npm run openai:verify-credentials` already loads them: the command fills `OPENAI_API_KEY` from `.secrets/openai-dev.env` (after `apps/backend/.env`, from a linked worktree's main checkout when the worktree has no copy; `scripts/lib/local-openai-env.mjs`), never overriding a value the environment already carries, so the owner exports nothing by hand. The confirmation flag is the consent that selects `ASK_AI_PROVIDER=openai` when a key is present and the variable is unset; an explicit `ASK_AI_PROVIDER=mock` still refuses
  - with the confirmation flag but no key anywhere, or with `ASK_AI_PROVIDER` set to anything but `openai`, it fails with an actionable message naming what is missing, not a stack trace from inside the provider factory
...
- Acceptance Criteria:
  ...
  - the full transcripts — assembled prompt, the cards attached to the lookup, what System 3 did (`retrieval`: semantic or lexical, the rule ids it attached, whether a gold rule was among them), model answer, reference worked solution, assertions, axis scores, and judge rationale per case per leg (`writeTranscript`), plus the side-by-side ranking rationale per case per cap (`writeRankingTranscript`) — are written to `output/answer-quality/`, which is gitignored, following the existing `output/prompt-preview/`, `output/retrieval-relevance-report.txt`, and `output/combo-answer-quality/` convention
...
- Notes:
  ...
  - measured 2026-09-07 (build): `artifact.test.ts` (15 tests) proves the round-trip, the no-prose guard (including a refusal test with a leaked `rationale` field), the replace-not-append behavior, both transcript writers, and every branch of `compareRuns` (identical lineup, model comparison, and each of the four incomparable reasons). As of 2026-09-07, the committed `results.json` holds run 3 of the first live bake-off (`gitCommit b3f860f`, the 18-case gold set, all four lineup models at both excerpt caps) — the first live runs replaced the build-time placeholder wholesale
...
- Notes:
  - scope choice made at the gate: full refresh (run the existing
    ...
    prices-only (a smaller, prices-only download and build — smaller git churn,
    more new code, drops rulings/combos/rules from the cadence). Written for
    full refresh
  - the backend price move shipped after this requirement was gated (the
    `trade-balancer-price-slim` work): the retired `build-card-prices.mjs` and
    the ~38 MB frontend `cardPrintingPrices.json` are gone, so the prices-only
