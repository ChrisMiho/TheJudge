...
### REQ-146
- Title: Card Collection destination and feature home
- Priority: high
- Description: The card collection manager is reachable as its own feature-portal destination whose home screen offers exactly two primary actions — scan cards, or view the collection.
- Acceptance Criteria:
  - the collection is registered in `PORTAL_DESTINATIONS` with a stable id, a flat top-level path, and a `React.lazy` boundary, following the shape every other destination uses (REQ-140)
  - the Menu lists it as one destination row; no other suite chrome changes
  - the destination home presents exactly two primary actions, **Scan cards** and **View collection**; no third primary action competes with them
  - opening the destination performs no backend call and no `AskAiRequest`
  - an unknown or stale stored destination id still resolves per existing routing rules; the collection never becomes the bare-`/` default
- Constraints:
  - frontend-only; no change to `AskAiRequest`, Zod schemas, `GameContext`, prompt assembly, the provider boundary, or any product-facing endpoint (DEC-010)
  - keep-alive destination mounting is preserved; the collection must not force sibling destinations to unmount
- Dependencies:
  - DEC-095
  - DEC-157
  - DEC-161
  - REQ-140
- Notes:
  - the two-action home is the IDEA's explicit framing; the collection overview is reached through it rather than replacing it

### REQ-147
- Title: Collection overview pie and total value
- Priority: high
- Description: The collection overview shows how the player's cards are distributed across their lists and what the whole collection is worth, in one read.
- Acceptance Criteria:
  - the overview renders a pie whose slices are **card counts per list** (each list's share of total card count), one distinct color per list
  - the **total collection USD value** renders in the center of the pie, computed as the sum of every entry's `qty × (foil ? usdFoil : usd)` across every list
  - selecting a list — from the chart or its adjacent legend/list — opens that list for view and edit
  - a collection with no lists renders an explicit empty state pointing at the scan action rather than an empty chart
  - the price-snapshot date is surfaced as date-level copy so the total never reads as a live quote (REQ-145)
- Constraints:
  - the chart is drawn with inline SVG/CSS; **no charting or visualization dependency may be added**
  - totals must reuse the shipped pure pricing selectors rather than restating the formula
  - USD-only, consistent with NFR-013
- Dependencies:
  - DEC-161
  - DEC-088
  - REQ-066
  - REQ-145
- Notes:
  - counts drive slice size while value fills the center — the IDEA's explicit split; a value-weighted pie is a different product and is not this requirement

### REQ-148
- Title: Batch scan, printing correction, and commit to one list
- Priority: high
- Description: Cards enter the collection as a scanned batch that the player can correct before committing it, whole, to a single list.
- Acceptance Criteria:
  - scanned cards accumulate into a review batch; each batch item defaults to the **scanned printing** (DEC-070 provenance)
  - a wrong printing is corrected in the review UI through the shared printing picker, without re-scanning the card
  - committing assigns the **entire batch to exactly one list** — an existing list, or a new list named and labelled at commit time — after which the batch clears
  - a scan batch may target an existing list, adding to it rather than only creating new lists
  - a card with **no price** in the snapshot is still added, recorded at $0 with the caution-triangle treatment, and is never silently dropped
  - abandoning a batch discards it without writing to any list
  - duplicates within a list are allowed; the stack duplicate-block and 10-card cap do not apply
- Constraints:
  - reuse the shipped scan stack and the shipped printing picker; do not fork either
  - when the camera is unavailable the scan surface closes, the reason is surfaced, and the rest of the feature stays usable (DEC-050)
  - printing choice stays a pricing/display concern and never reaches prompt context, rulings, or a request payload
- Dependencies:
  - DEC-161
  - DEC-050
  - DEC-070
  - REQ-065
- Notes:
  - the Trade Balancer drops an unpriced scanned card because it is a value comparator; a collection is an inventory record, so the opposite behavior is correct here

### REQ-149
- Title: Browser-local collection working copy
- Priority: high
- Description: The player's collection survives leaving and returning to the app without forcing a re-import every visit.
- Acceptance Criteria:
  - the collection persists to browser `localStorage` under a namespaced `thejudge.collection.*` key, written on mutation and read on destination mount
  - a returning player with a valid stored collection sees it restored, with no import prompt and no re-scan
  - stored data is validated field-by-field on read; malformed, partial, or unparseable data degrades to a clean empty collection and never throws or white-screens
  - the stored payload carries an explicit schema version
  - a storage-quota or write failure surfaces an explicit error directing the user to export, rather than failing silently
  - no cookie is introduced; the presence of a valid stored collection is itself the signal that no import is needed
- Constraints:
  - frontend-only, single-device; no account, no server-side store, no cross-device sync
  - no product cap on list or entry count
- Dependencies:
  - DEC-162
  - DEC-103
- Notes:
  - follows the `lib/lifeTracker/persistence.ts` validate-or-default pattern established by DEC-103

### REQ-150
- Title: Master backup export and restore
- Priority: high
- Description: The player owns a durable master file of their collection, is prompted to keep it current, and can restore from it.
- Acceptance Criteria:
  - export writes a single JSON master file containing the schema version, list definitions with category labels, entries as printing id + quantity + foil, and an export timestamp
  - after a successful batch commit the app offers the export with a plain **beta warning** that browser storage can be cleared and the file is the only durable copy; the prompt is dismissible and never blocks continuing
  - import reads a previously exported file and **replaces** the working copy, behind an explicit confirmation naming the current list and card counts being replaced
  - an unreadable, wrong-shaped, or wrong-version file is rejected with a plain error and leaves the working copy untouched
  - prices are **not** stored in the master file; entries carry printing ids and are re-priced from the committed snapshot on load
- Constraints:
  - export/import are entirely in-browser; no upload, no server, no external service
  - restore is replace-semantics, not merge; a destructive restore must never be silent
- Dependencies:
  - DEC-162
  - REQ-149
- Notes:
  - storing printing ids rather than prices means a refreshed snapshot re-values an existing collection instead of freezing stale prices into the backup

### REQ-151
- Title: List management and entry editing
- Priority: medium
- Description: A list is a plain named container with an inert category label, and its entries stay fully editable after the batch that created them.
- Acceptance Criteria:
  - a list carries a user-chosen name and one category label — `folder`, `deck`, or `box`
  - the label is presentation only: no capacity, size limit, format legality, commander rule, or any other behavior differs between labels
  - a list can be created, renamed, re-labelled, and deleted; deleting a non-empty list confirms first
  - within a list, an entry supports change printing, change quantity, toggle foil, and remove
  - list and entry edits update the overview totals and pie immediately
- Constraints:
  - no deck/box gameplay rules of any kind (DEC-013)
  - entry edits reuse the shared printing picker and pricing selectors
- Dependencies:
  - DEC-161
  - REQ-147
  - REQ-148
- Notes:
  - labels-only is the IDEA's explicit decision; per-label behavior would be the deterministic-rules drift DEC-013 forbids
