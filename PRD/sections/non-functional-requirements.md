...
- Notes:
  ...
  - re-measured 2026-09-05 for REQ-181: `onnxruntime-node` bundles all three platforms' native binaries directly in its published package (not npm `optionalDependencies`), so every install pulls ~283MB before pruning; `scripts/package-lambda.sh` prunes every platform except the one the Lambda loads before zipping — linux/arm64, the architecture `scripts/aws-bootstrap.sh` creates the function with (`scripts/lib/prune-onnxruntime-platforms.mjs`). Corrected 2026-09-07: the 2026-09-05 pruning kept linux/x64 and deleted linux/arm64, so the deployed package had no binding the function could load; the size arithmetic below was unaffected because the two bindings are the same size. The re-measured real non-data footprint (pruned production `node_modules` + the warmed local-model cache, ~130 MB) raised the reserve from 20 MB to 130 MB, shrinking the data budget from 230 MB to 120 MB against the then-current ~117 MB tracked data. Corrected again 2026-09-07, after the first deploy of that fix: the cold start then failed one module later — `sharp`, which `@huggingface/transformers` imports at load, had no linux-arm64 build because npm installs its optional platform packages for the host running `npm ci` (the deploy runner is linux/x64), so `scripts/package-lambda.sh` now installs with `--os=linux --cpu=arm64 --libc=glibc` and refuses without `@img/sharp-linux-arm64` (`scripts/lib/assert-sharp-platform-binding.mjs`); sharp's arm64 build plus libvips is ~17 MB, in place of the host build the earlier measurement already counted, so the reserve is unchanged
...
- Constraints:
  - the build records before/after gzipped sizes and asserts the trimmed `cardMetadata.json` (up-front fields only) is at least 40% smaller (gzipped) than the prior combined artifact — a relative gate, so acceptance does not hinge on an estimated byte ceiling and stays correct as the Scryfall corpus grows; the measured slim size against the current 33,399-card corpus is ~1.8 MB gzipped (~57% reduction, re-measured after REQ-174/Slice C dropped the stored `imageUrl` string in favor of a representative-printing id), so the ≥40% floor carries headroom for data-refresh drift while still failing on a first-load regression
...
- Notes:
  ...
  - owner-recalibrated 2026-09-04: the original 80% figure was derived from raw-byte proportions but stamped onto a gzipped gate; the removed descriptive text compresses well while the kept `cardId`/`imageUrl` barely did, so raw size drops ~87% but gzipped drops only ~48.1% (4.25 MB → 2.20 MB, measured against the live 33,399-card corpus). The relative-gate shape is kept (robust to corpus growth); only the floor moved, to ≥40%, below the measured ~48% with headroom for data-refresh drift while still failing a real first-load regression
  - re-measured after REQ-174/Slice C (trade-balancer-price-slim): the slim shape now stores a representative-printing id (`imageId`) instead of the full `imageUrl` string, so it compresses further — gzipped drops ~57.2% (4.23 MB → 1.81 MB, measured against the same live 33,399-card corpus). The ≥40% floor is unchanged and still has headroom
