...
- Status: planned
- Summary: Frontend-only feature-portal destination for a durable local record of physically owned cards. A two-action home (Scan cards / View collection) leads to an overview presenting a per-list card-count pie — one color per list, drawn with inline SVG/CSS and no charting dependency — with total collection USD centered inside it; selecting a list opens it for edit. Cards enter as a scanned batch that defaults each item to the scanned printing (DEC-070), allows printing correction through the shipped `PrintingPicker`, and commits the whole batch to exactly one list (existing or created at commit time) before clearing. Lists are one type with an inert `folder`/`deck`/`box` category label; entries support printing change, quantity, foil, and removal, and an unpriced card is kept at $0 with the DEC-087 caution treatment rather than dropped. Durability is two-tier: a validated, schema-versioned `localStorage` working copy (no cookie — a valid stored copy is itself the "don't force an import" signal) plus a user-owned JSON master backup file, prompted after each batch commit with a beta warning, whose import replaces the working copy behind explicit confirmation. Entries store printing ids, not prices, so a refreshed snapshot re-values an existing collection. Contract-frozen and non-integrating: no backend call, no prompt/rulings involvement, and no seeding into In-Depth zones or the Trade Balancer in v1.
- Lives in: (planned) `apps/frontend/src/components/portal/card-collection/` and `apps/frontend/src/lib/collection/`, registered in `apps/frontend/src/components/portal/destinationRegistry.tsx`. Reuses the shipped Trade Balancer pricing primitives (`lib/trade/loadCardPrices.ts`, `lib/trade/pricing.ts`, `components/trade/PrintingPicker.tsx`) and the shipped scan stack (`hooks/useScanCapture.ts`, `components/ScanCameraSurface.tsx`, `lib/scan/resolveScanCandidates.ts`, `lib/scan/loadScanMap.ts`), with `components/trade/useTradeScan.ts` as the per-surface scan-adapter precedent and `lib/lifeTracker/persistence.ts` as the validate-or-default persistence precedent.
- Backed by: DEC-161, DEC-162, REQ-146, REQ-147, REQ-148, REQ-149, REQ-150, REQ-151, FLOW-019, FLOW-020, NFR-015
