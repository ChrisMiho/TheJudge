...
- Status: shipped
- Summary: A feature-portal destination providing a live full-table life/counter tracker for 2–8 players: rotated per-seat life cards with grid/list seat arrangements, widened edge `+`/`−` life zones, a per-opponent commander-damage matrix (always-visible `−`/`+` bands) plus the full named-counter palette and custom counters, commander damage always reducing life, a life ≤ 0 skull death cue (visual only, no elimination), always-on game-wide day/night flip in the header (manual only; not seeded into In-Depth), basic game setup (`−`/`+` count, 20/25/30/40 + Custom defaulting to 60, In-Depth-matching 2→20 / 3+→40 defaults, Edit names, reset), and browser-local persistence across reload/phone-lock. Switching directly from the tracker into In-Depth Question one-way seeds the current roster (count, names, life, counters) into `GameContext` via additive optional per-player fields; edits in Assistant never write back to tracker state. Visual-refinement work (including always-on day/night, tap-zone sizing, and Game Setup count/life defaults) is tracked under `PRD/work/player-life-tracker-refinement/`.
