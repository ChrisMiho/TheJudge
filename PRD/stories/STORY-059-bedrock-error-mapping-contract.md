...
- title: Map OpenAI and runtime failures into stable ask-ai error codes, status mappings, and retry hints.
...
- scope:
  - classify OpenAI SDK/runtime failures into canonical error codes (`VALIDATION_ERROR`, `PROVIDER_UNAVAILABLE`, `PROVIDER_TIMEOUT`, `UNEXPECTED_ERROR`)
...
- acceptance criteria:
  - known OpenAI failure classes resolve to stable code/status semantics
...
- dependencies:
  - `STORY-058` - OpenAI invocation path must exist before provider-specific failure mapping can be completed
