...
| 8 | close | sonnet | ok | `0 → 31` | commit `7293dfd` on `thejudge-auto/rule-excerpt-cap-ten-work` (pushed) — open-PR path, four pre-merge checks passed (HEAD = remote tip; PR https://github.com/ChrisMiho/TheJudge/pull/231 OPEN, base `main`, `mergedAt` null before and after; 9/9 criteria `true` read from the files; no runtime to clean up). Receipt `PRD/instructions/receipts/rule-excerpt-cap-ten-2026-09-10.md` written before the delete with `## Graph run` (both ledgers verbatim) and `## Intake`; durable truth confirmed present, nothing promoted twice; system-map not flipped (the 14 blocks already rewrote the three System 3 entries; no planned/partial marker existed); retired `decisions.md:73` (DEC-032) named as a known leftover, not edited; `PRD/work/STATUS.md` stripped; `git rm -r PRD/work/rule-excerpt-cap-ten/`; `npm run quality:check` green before and after staging; no worktree or branch removed. Run ends COMPLETE; `land` is the owner's merge and gets no row | 2026-09-10 |
