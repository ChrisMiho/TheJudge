...
- Autonomous graph runs: two owner-triggered halves joined by `main`. Start a fresh idea with `/graph-kickoff "<request>"` — which dispatches `graph-preflight` itself as its first step and stops at quality-check PASS with a docs-only proposal PR. Answer the `GATE-QUESTIONS.md` verdict slots in that PR and merge it to `main`; that merge is the "build it" signal. `/loop graph-implement` — a single background loop — then picks up each approved-and-merged spec in its own `.worktrees/implement-<slug>` cut from `origin/main`, applies the verdicts via `graph-gate-review`, builds it, runs cleanup on that branch, and opens a code PR into `main` you merge; that PR carries the receipt and the work-folder deletion, so a package costs exactly two merges and nothing follows yours. Every kickoff works in its own `.worktrees/kickoff-<slug>` off `origin/main` and never touches your checkout, so a second idea can start while the first waits at its gate; two ideas *at the same moment* need two sessions in two checkouts (`OPERATOR.md`, recipe 2). Contract in `instructions/graph-workflow-contract.md`, permission profile in `.claude/graph-profile.json` — which binds only in a session launched with `claude --settings .claude/graph-profile.json` and is inert without that flag. Owner-facing task recipes: `OPERATOR.md`.
...
- Runtime code is split across `apps/frontend` and `apps/backend`, with a main product-facing backend route (`POST /api/ask-ai`), two read-only retrieval routes (`GET /api/cards/:oracleId`, `GET /api/cards/:oracleId/prices`), and a health endpoint (canonical rule: NFR-004).
