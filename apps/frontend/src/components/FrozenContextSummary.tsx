...
export function FrozenContextSummary({ frozenGameContext }: FrozenContextSummaryProps): JSX.Element {
  ...
  return (
    <section
      ...
      className="rounded-2xl border border-slate-700/70 bg-slate-900/55 p-4"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          {activePlayer && (
            <p className="text-sm text-slate-300">
              <span className="font-medium text-slate-200">Active player:</span>{" "}
        ...
        <button
          ...
          className="shrink-0 rounded-lg border border-slate-600 bg-slate-800/70 px-3 py-1.5 text-xs font-semibold text-slate-200 transition hover:bg-slate-700/80"
        >
      ...
      {expanded && (
        <div className="mt-4 space-y-4 border-t border-slate-700/60 pt-4">
          {players.length > 0 && (
            <div className="space-y-1">
              <p className="text-xs font-semibold uppercase tracking-[0.08em] text-slate-400">Setup</p>
                ...
                {cards.map((card) => {
                  ...
                  return (
                    <li
                      ...
                      className="space-y-1 rounded-xl border border-slate-700/60 bg-slate-900/50 p-3 text-sm text-slate-300"
                    >
                      <p className="font-semibold text-slate-100">{card.name}</p>
                      {card.typeLine && <p className="text-xs text-slate-400">{card.typeLine}</p>}
                      {card.oracleText && (
                        <p className="text-xs text-slate-400">{card.oracleText}</p>
                      )}
