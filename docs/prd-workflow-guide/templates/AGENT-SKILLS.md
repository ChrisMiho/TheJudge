...
Run the sync after any skill change, then commit both trees together. They are
...
literal command, prefixed `/proj-*` (Claude Code) or `$proj-*` (Codex),
...
3. Verify: `diff -rq .claude/skills .agents/skills` — must produce no output.
4. Commit both skill trees together.
...
- `.claude/skills/proj-kickoff/reference.md` — PRD quick map
