...
3. Derive the shared branch as `thejudge-auto/<slug>` unless the user supplied a branch or PR. **If the resolved shared branch equals the recorded autonomous base, block before worktree creation and report it: a pull request's head and base cannot be the same ref, so this branch can never open a clean PR.** Supply a distinct branch — conventionally `<recorded-base>-work` — and restart. The collision arose whenever the recorded base was itself a `thejudge-auto/<slug>` branch, which every graph run's base was until 2026-09-06; the graph's base is `origin/main` now (REQ-193), so under `graph is controlling` this guard never fires and the explicit-shared-branch check in `SKILL.md` `## Mode` stands in for it. The guard stays for direct use. Left unguarded, the skill would improvise — pushing some slices straight onto the base and the rest onto an ad-hoc fork — so the PR never shows the full deliverable (observed on `life-tracker-spec` PR #105 and `user-feedback-spec` PR #107).
4. Create a unique local contributor branch and an isolated worktree at `.worktrees/implement-<slug>`. Multiple worktrees cannot check out the same local branch. Refuse to create or adopt a worktree at any path outside the repo-local `.worktrees/` root — a sibling directory such as `../<repo>-worktrees/`, a temp/scratchpad path, or an absolute path elsewhere on disk are all blockers, not adoptable states. This mirrors the identical preflight refusal `thejudge-prepare` applies to preparation worktrees. **Under `graph is controlling`, adopt instead of create:** the dispatch's `Working directory:` names `.worktrees/implement-<slug>`, already a worktree on the shared branch the driver cut from `origin/main`; confirm `git branch --show-current` there equals the supplied shared branch and work in place, creating no branch and no worktree.
5. Base the contributor branch on `origin/<shared-branch>` when it exists; otherwise use `origin/<recorded base>`. In place under the graph there is no contributor branch: the shared branch itself is checked out and tracks `origin/<shared-branch>`.
6. Confirm the selected GAMEPLAN, slice docs, and relevant baseline exist unchanged at that remote start point — `origin/<shared-branch>`'s tip when it exists (under the graph the driver's publish-before-build commit is that tip), otherwise `origin/<recorded base>`. If the launch checkout has relevant modified or untracked inputs, block and report them; never copy, stash, or commit them implicitly.
7. Record the launch checkout's status and do not alter, stage, stash, or commit its files. Under the graph the launch checkout is the root the build worktree hangs off — not the worktree you work in — and the driver compares its `git status --porcelain` before and after this node (REQ-193).
